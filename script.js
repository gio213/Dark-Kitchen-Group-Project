...
// checkbox category event listener
...
for(let i of dietUl.children){
  i.children[0].id=i.children[1].textContent;
  let d = document.getElementById(i.children[0].id);
  d.addEventListener("click", () => checkCategory(i.children[0],i.children[0].id))

}
for (let li of categoriesUl.children) {
  li.children[0].id=li.children[1].textContent;
  let country = document.getElementById(li.children[0].id);
  country.addEventListener("click", () => checkCategory(li.children[0], li.children[0].id));
}
